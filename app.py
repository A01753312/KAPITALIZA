...
# Cambios:
# - Antecedentes = "Monto Dispuesto" del Excel.
# - ZIP del grupo incluye pagarés individuales de cada integrante.
...
def crear_contexto_grupal(grupo_df: pd.DataFrame, datos_grupo: dict) -> dict:
    """
    - ANTECEDENTES se toman de la columna 'Monto Dispuesto' del Excel.
    - Llaves por integrante: MontoAntecedente, MontoAntecedente_FORMAT (compatibles con la plantilla).
    """
    total_pagare = 0.0
    ...
    for _, row in grupo_df.iterrows():
        ...
        # Pagaré (Excel)
        monto_raw = (
            pick_col(row, ["Monto Pagaré","Monto Pagare","Monto Pagaré "], contains=["monto pagar"])
            or pick_col(row, ["CUOTA","Monto","Importe"], contains=["cuota","monto","importe"])
        )
        monto_pagare = parse_money(monto_raw)

        # Antecedente desde Excel: 'Monto Dispuesto'
    ...
    ctx = {
        ...
        # Totales de ANTECEDENTES (para la plantilla)
...
TEMPLATES = {
    ...
    "CONVENIO":   TPL_DIR / "CONVENIO_GRUPALejefinal10.docx",  # tu plantilla
}
            ...
            if st.button("🚀 Generar Pagarés (Excel)"):
                with st.spinner("Generando pagarés desde Excel..."):
                    try:
                        ...
                        total, errors = 0, []
                        # Detectar bloques KGRUPAL para excluirlos (solo pagarés fuera de grupos)
...
with tab2:
    ...
    if df.empty:
        ...
        if not grupos_kgrupal:
            ...
            # Mostrar integrantes base con Antecedente = Monto Dispuesto
            ...
            for _, row in grupo_df.iterrows():
                ...
                integrantes_data.append({
                    ...
                    "Folio": folio,
                    "Monto Pagaré": monto,
                    "Monto Dispuesto (Antecedente)": antecedente,
                })
            ...
            if st.button("🚀 Generar Convenio para este Grupo", key=f"btn_grupo_{gidx}"):
                with st.spinner("Generando convenio grupal..."):
                    try:
                        ...
                        # Contexto con antecedentes desde "Monto Dispuesto"
                        ctx_grupal = crear_contexto_grupal(grupo_df, datos_grupo)
                        ...
                        if not tpl_path or not tpl_path.exists():
                            ...
                            # Crear ZIP con el DOCX (imágenes) + Pagarés individuales
                            zip_buffer = io.BytesIO()
                            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
                                # Convenio y anexos
                                ...
                                # =========== NUEVO: Generar Pagarés Individuales del grupo ===========
                                for _, row in grupo_df.iterrows():
                                    # Contexto individual desde la fila
                                    ctx_indv = row_to_context(row)

                                    # Elegir plantilla por sucursal detectada (o fallback a HUEHUETOCA)
                                    ...
                                    if not tpl_path_indv or not tpl_path_indv.exists():
                                        continue  # si no hay plantilla, saltamos
                                    ...
                                    try:
                                        ...
                                        # Guardar en subcarpeta dentro del ZIP
                                        arcname = f"PAGARES_INDIVIDUALES/{tpl_key_indv}/{folio_cli}_{nombre_cli}_{tpl_key_indv}.docx"
                                        zf.writestr(arcname, docx_ind)
                                    except Exception:
                                        pass

                            zip_buffer.seek(0)

                            st.success(f"✅ Convenio (con pagarés individuales) generado para {grupo_sel}")
                            st.download_button(
